...
class MGDANet(MultiTaskNet):
    ...
    def cal_weights(self, batch):
        ...
        if self.approximate_norm_solution:
            ...
            rep = self.backbone(images_volatile)
            ...
            for t in self.task_names:
                optimizer.zero_grad()
                out_t = self.heads[t](rep_variable)
                loss = self.heads[t].losses(out_t, labels[t])
            ...
            for t in self.task_names:
                ...
                optimizer.zero_grad()
                rep = self.backbone(images)
                out_t = self.heads[t](rep)
                loss = self.heads[t].losses(out_t, labels[t])
                loss_data[t] = loss.data
                loss.backward()
                grads[t] = []
        ...
        weights = {}
        sol, min_norm = MinNormSolver.find_min_norm_element([grads[t] for t in self.task_names])
        for i, t in enumerate(self.task_names):
            weights[t] = float(sol[i])
        return weights

    def training_step(self, batch, batch_idx):
        img, targets = batch

        weights = self.cal_weights(batch)

        preds, shared = self(img)
        # print(img.dtype, preds[0].dtype, preds[1].dtype) # To This step, data.dtype is float32
        losses = [
            self.heads[h].losses(preds[i], targets[i]) * weights[h]
        ...
        self.log_dict(log_weights, prog_bar=True, sync_dist=True)
        return loss
