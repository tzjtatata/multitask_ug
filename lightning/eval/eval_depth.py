...
class LorenDepthMeter(object):
    ...
    def update(self, pred, gt):
        ...
        invalid_idx = -1
        # 不知道为什么pred的shape是: (N, 256, 512, 1)
        pred = pred.squeeze().unsqueeze(1)
        valid_mask = (torch.sum(gt, dim=1, keepdim=True) != invalid_idx)
        abs_err = torch.mean(torch.abs(pred - gt).masked_select(valid_mask)).item()
        self.total_abs_err = r * self.total_abs_err + (1 - r) * abs_err
    ...
    def get_score(self, verbose=True):
        eval_result = dict()
        eval_result['abs_err'] = self.total_abs_err
...
def eval_depth_predictions(database, save_dir, overfit=False):
    ...
    if database == 'NYUD':
        ...
        gt_set = 'val'
        db = NYU_MT(split=gt_set, do_depth=True, overfit=overfit)
