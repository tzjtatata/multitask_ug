...
CITY_CATEGORY_NAMES = ['road', 'sidewalk', 'building', 'wall', 'fence', 'pole', 'traffic light', 'traffic sign',
                       'vegetation', 'terrain', 'sky', 'person', 'rider', 'car', 'truck', 'bus', 'train', 'motorcycle',
                       'bicycle']


CITY_PART_CATEGORY_NAMES = ['background', 'human/rider torso', 'human/rider head', 'human/rider arms', 'human/rider legs', 'car/truck/bus windows',
                            'car/truck/bus wheels', 'car/truck/bus lights', 'car/truck/bus license_plate', 'car/truck/bus chassis']
...
class PartSemsegMeter(SemsegMeter):
    def __init__(self, database):
        super(PartSemsegMeter, self).__init__(database)
        n_classes = 9
        cat_names = CITY_PART_CATEGORY_NAMES
        has_bg = True
...
def eval_semseg_predictions(database, save_dir, overfit=False):
    ...
    if database == 'PASCALContext':
        ...
        from data.nyud import NYU_MT
        n_classes = 40
        cat_names = NYU_CATEGORY_NAMES
        has_bg = False
        gt_set = 'val'
        db = NYU_MT(split=gt_set, do_semseg=True, overfit=overfit)
