...
class MinNormSolver:
    ...
    def find_min_norm_element(vecs):
        ...
        # print("Line106 Type of init_sol, dps:", type(init_sol), type(dps))
...
class MGDA(BaseBalancer):

    @configurable
    def __init__(
        self,
        approximate_norm_solution,
        normalization_type='none',
        **kwargs
    ):
        super(MGDA, self).__init__(**kwargs)
    ...
    def from_config(cls, cfg):
        ...
        ret['approximate_norm_solution'] = cfg.balancer.is_approximate
        if 'normalization_type' in cfg.balancer:
            ret['normalization_type'] = cfg.balancer.normalization_type
        return ret
    
    def cal_weights(self, model, images, labels):
        loss_data = {}
        grads = {}
        ...
        if self.approximate_norm_solution:
            ...
            rep = model.backbone(images_volatile)
            ...
            for t in self.task_names:
                model.zero_grad()
                out_t = model.heads[t](rep_variable)
                loss = model.heads[t].losses(out_t, labels[t])
            ...
            for t in self.task_name:
                ...
                model.zero_grad()
                # rep = model.backbone(images)
                # out_t = model.heads[t](rep)
                rep, out_t = model._forward_by_task(images, t)
                loss = model.heads[t].losses(out_t, labels[t])
                loss_data[t] = loss.data
                loss.backward()
                grads[t] = []
        ...
        weights = []
        sol, min_norm = MinNormSolver.find_min_norm_element([grads[t] for t in self.task_name])
        for i, t in enumerate(self.task_name):
            weights.append(float(sol[i]))
        self.params.data = torch.tensor(weights).to(self.params.device)
    ...
    def run(self, losses, **kwargs):
        loss = super(MGDA, self).run(losses)
