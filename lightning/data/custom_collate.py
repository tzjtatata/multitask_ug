...
def collate_mil_for_MuST(batch):
    """
    Puts each data field into a tensor with outer dimension batch size.
    Custom-made for supporting MIL
    """
    ...
    if isinstance(batch[0], torch.Tensor):
        ...
        full_keys = []
        for d in batch:
            full_keys += list(d.keys())
        full_keys = list(set(full_keys))  # 去重
        batch_modified = {key: collate_mil_for_MuST([d[key] for d in batch if key in d]) for key in full_keys if key.find('idx') < 0}
        if 'edgeidx' in full_keys:
            batch_modified['edgeidx'] = [batch[x]['edgeidx'] for x in range(len(batch))]
        return batch_modified
