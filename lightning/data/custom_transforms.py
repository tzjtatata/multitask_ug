...
import cv2
import math
import lightning.utils.helpers as helpers
import torchvision
...
class ScaleNRotate(object):
    ...
    def __call__(self, sample):
        ...
        for elem in sample.keys():
            ...
            if 'label_map' in elem:
                # 不做旋转和切割，而是把旋转的坐标放进label_map里存起来
                # 但是现在不支持旋转：原本的label_map也不支持旋转的操作:因为不能用普通的方式去求label map，要用roi align.
                label_map = sample[elem]
                _h, _w = h *sc, w*sc
                _i, _j = h * (1-sc)/2, w*(1-sc) / 2
                coords = (_i / h,
                          _j / w,
                          _h / h,
                          _w / w)
                # trick to store coords_map is label_map
                label_map[-1,0,0,0],label_map[-1,0,0,1],label_map[-1,0,0,2],label_map[-1,0,0,3] = coords
                sample[elem] = label_map
                continue
...
class FixedResize(object):
    ...
    def __call__(self, sample):
        ...
        for elem in elems:
            if 'meta' in elem or 'bbox' in elem or 'label_map' in elem:
                continue
    ...
    def __call__(self, sample):
        ...
        if random.random() < 0.5:
            for elem in sample.keys():
                ...
                if 'label_map' in elem:
                    tmp = sample[elem]
                    # tmp: (3, 5, h, w) -> (2, 5, h, w) -> (2*5, h, w) -> (h, w, 2*5) 才能被cv2.flip使用
                    _tmp = tmp[0:2].reshape(-1, tmp.shape[2], tmp.shape[3]).transpose((1, 2, 0))
                    _tmp = cv2.flip(_tmp, flipCode=1)
                    # _tmp: (h, w, 2*5) ->(2*5, h, w) -> (2, 5, h, w)再赋值回tmp的前两维
                    tmp[0:2] = _tmp.transpose((2, 0, 1)).reshape(2, tmp.shape[1], tmp.shape[2], tmp.shape[3])
                    sample[elem] = tmp
                    continue
                else:
                    tmp = sample[elem]
