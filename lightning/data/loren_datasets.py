...
import numpy as np
import hydra
import lightning.data.panoptic_parts as pp
...
# Loren version of NYUv2 dataset which use different augmentation and resolutions.
...
class CityScapes(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        data_dict = {'im': image, 'semseg': seg, 'part_seg': part_seg, 'depth': disparity, 'noise': noise}
        ...
        im = 2. * data_dict.pop('im') - 1.  # normalised to [-1, 1]
        data_dict['image'] = im
        data_dict['part_seg'] = data_dict['part_seg'].unsqueeze(0)
        return data_dict
...
class CIFAR100MTL(CIFAR100):
    ...
    def __init__(self, root, subset_id=0, train=True, transform=None, target_transform=None, download=False):
        ...
        self.class_dict = {
            "aquatic mammals": ["beaver", "dolphin", "otter", "seal", "whale"],  # 0
            ...
            "people": ["baby", "boy", "girl", "man", "woman"],  # 14
            "reptiles": ["crocodile", "dinosaur", "lizard", "snake", "turtle"],  # 15
            "small mammals": ["hamster", "mouse", "rabbit", "shrew", "squirrel"],  # 16
            "trees": ["maple_tree", "oak_tree", "palm_tree", "pine_tree", "willow_tree"],  # 17
...
class LorenCityscapes(pl.LightningDataModule):
    ...
    def from_config(cls, cfg):
        ...
        try:
            data_dir = os.environ.get(
                'CITYSCAPES_ROOT')  # Different os has different handler, while some may raise error, some may not.
        except:
            data_dir = '/data2/dataset/cityscapes_loren'
        if data_dir is None:
            data_dir = '/data2/dataset/cityscapes_loren'
